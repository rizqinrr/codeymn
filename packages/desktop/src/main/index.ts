...
const TEST_ONBOARDING = process.env.CODEYMN_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.CODEYMN_DISABLE_EMBEDDED_WEB_UI = "true"
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Codeymn Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("codeymn://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("codeymn")
  registerRendererProtocol()
  setDockIcon()
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.CODEYMN_PORT
    if (fromEnv) {
