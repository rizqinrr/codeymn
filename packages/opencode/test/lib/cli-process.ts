...
//   - CODEYMN_CONFIG_CONTENT      : provider config inline, no files to find
//   - CODEYMN_TEST_HOME           : pins os.homedir() → tmpdir
//   - CODEYMN_DISABLE_PROJECT_CONFIG : skip walking up for opencode.json
//   - CODEYMN_PURE                : skip external plugin discovery + install
//   - CODEYMN_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CODEYMN_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CODEYMN_CONFIG_CONTENT: configJson,
    CODEYMN_DISABLE_PROJECT_CONFIG: "1",
    CODEYMN_PURE: "1",
    CODEYMN_DISABLE_AUTOUPDATE: "1",
    CODEYMN_DISABLE_AUTOCOMPACT: "1",
    CODEYMN_DISABLE_MODELS_FETCH: "1",
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CODEYMN_CONFIG_CONTENT: JSON.stringify({
