...
const original = {
  ...
  envPassword: process.env.CODEYMN_SERVER_PASSWORD,
  envUsername: process.env.CODEYMN_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.CODEYMN_SERVER_PASSWORD
  else process.env.CODEYMN_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.CODEYMN_SERVER_USERNAME
  else process.env.CODEYMN_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.CODEYMN_SERVER_PASSWORD = auth.password
  Flag.CODEYMN_SERVER_USERNAME = auth.username
  process.env.CODEYMN_SERVER_PASSWORD = auth.password
  process.env.CODEYMN_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.CODEYMN_SERVER_PASSWORD = undefined
  Flag.CODEYMN_SERVER_USERNAME = auth.username
  delete process.env.CODEYMN_SERVER_PASSWORD
  process.env.CODEYMN_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.CODEYMN_DISABLE_DEFAULT_PLUGINS
    process.env.CODEYMN_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.CODEYMN_DISABLE_DEFAULT_PLUGINS
      else process.env.CODEYMN_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
