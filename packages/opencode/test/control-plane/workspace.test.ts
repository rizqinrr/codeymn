...
const originalEnv = {
  CODEYMN_AUTH_CONTENT: process.env.CODEYMN_AUTH_CONTENT,
  CODEYMN_EXPERIMENTAL_WORKSPACES: process.env.CODEYMN_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CODEYMN_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CODEYMN_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CODEYMN_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CODEYMN_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CODEYMN_EXPERIMENTAL_WORKSPACES).toBe("true")
