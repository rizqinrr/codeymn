...
const managedConfigDir = process.env.CODEYMN_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.CODEYMN_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.CODEYMN_CONSOLE_TOKEN
  else process.env.CODEYMN_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})
...
it.effect("does not create global config when CODEYMN_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:CODEYMN_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.effect("does not try to install dependencies in read-only CODEYMN_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("CODEYMN_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible CODEYMN_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("CODEYMN_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing CODEYMN_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable CODEYMN_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "CODEYMN_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
// Note: preload.ts sets CODEYMN_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("CODEYMN_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { CODEYMN_CONFIG_DIR: undefined, CODEYMN_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "CODEYMN_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { CODEYMN_DISABLE_PROJECT_CONFIG: "true", CODEYMN_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed CODEYMN_PERMISSION JSON used to crash
...
describe("CODEYMN_PERMISSION env var", () => {
  it.instance("does not crash when CODEYMN_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "CODEYMN_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("CODEYMN_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in CODEYMN_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "CODEYMN_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in CODEYMN_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
