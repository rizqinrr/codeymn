...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.CODEYMN_CONFIG
  delete process.env.CODEYMN_TUI_CONFIG
...
it.instance("project config takes precedence over CODEYMN_TUI_CONFIG (matches CODEYMN_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("CODEYMN_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from CODEYMN_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
