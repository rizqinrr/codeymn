...
When `plugin` entries exist in a writable `.opencode` dir or `CODEYMN_CONFIG_DIR`, OpenCode installs `@opencode-ai/plugin` into that dir and writes:
...
- `--pure` / `CODEYMN_PURE` skips external TUI plugins only.
