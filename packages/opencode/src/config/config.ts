...
function globalConfigFile() {
  const candidates = ["codeymn.jsonc", "codeymn.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.CODEYMN_CONFIG && !Flag.CODEYMN_CONFIG_DIR && !Flag.CODEYMN_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "codeymn.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "codeymn.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CODEYMN_CONFIG_CONTENT") return "local"
        ...
        if (Flag.CODEYMN_CONFIG) {
          yield* merge(Flag.CODEYMN_CONFIG, yield* loadFile(Flag.CODEYMN_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.CODEYMN_CONFIG })
        }

        if (!Flag.CODEYMN_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("codeymn", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.CODEYMN_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from CODEYMN_CONFIG_DIR", { path: Flag.CODEYMN_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir === Flag.CODEYMN_CONFIG_DIR) {
            for (const file of ["codeymn.json", "codeymn.jsonc"]) {
        ...
        if (process.env.CODEYMN_CONFIG_CONTENT) {
          const source = "CODEYMN_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CODEYMN_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from CODEYMN_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CODEYMN_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CODEYMN_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.CODEYMN_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CODEYMN_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("CODEYMN_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.CODEYMN_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.CODEYMN_DISABLE_PRUNE) {
