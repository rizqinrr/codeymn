...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "codeymn"),
    ...(await Filesystem.findUp(["codeymn.json", "codeymn.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "codeymn"))
  }
  if (Flag.CODEYMN_CONFIG) files.push(Flag.CODEYMN_CONFIG)
