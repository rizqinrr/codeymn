...
function useShareUrl() {
  return isMock() ? "https://dev.opencode.ai" : ""
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    response = await fetch("/exchange_github_app_token", {
      method: "POST",
      headers: {
