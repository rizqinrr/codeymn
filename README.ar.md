<p align="center">
  <a href="https://Codeymn.ai">
    <picture>
      ...
      <img src="packages/console/app/src/asset/logo-ornate-light.svg" alt="???? Codeymn">
    </picture>
  </a>
</p>
<p align="center">???? ????? ??????? ????????? ????? ??????.</p>
...
brew install Codeymn              # macOS ? Linux (???? brew ???????? ????? ???)
...
??? ??? ????? ????????? ?? Codeymn? ???? ????? [contributing docs](./CONTRIBUTING.md) ??? ????? pull request.
