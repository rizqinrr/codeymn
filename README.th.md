...
<p align="center">
  ...
  <a href="https://github.com/anomalyco/Codeymn/actions/workflows/publish.yml"><img alt="?????????????" src="https://img.shields.io/github/actions/workflow/status/anomalyco/Codeymn/publish.yml?style=flat-square&branch=dev" /></a>
</p>
...
mise use -g Codeymn               # ?????????????????????
nix run nixpkgs#Codeymn           # ???? github:anomalyco/Codeymn ?????????????????????
```
...
Codeymn ??????????????????????????????????? ?????????????????? [????????](https://github.com/anomalyco/Codeymn/releases) ???? [Codeymn.ai/download](https://Codeymn.ai/download)
...
??????????????????????????? Codeymn ???????? [???????????????????](./CONTRIBUTING.md) ??????? Pull Request
